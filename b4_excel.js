...
document.getElementById('fileInput').addEventListener('change', function(e) {
    ...
    reader.onload = function(e) {
        ...
        console.log('File data parsed:', fileData);
...
function calculateepsilonSH(cementType, t, tPrime, t0, T, h, fc, vS, c, wC, aC, specimenShape, aggregateType) {
    ...
    console.log('epsilonSH calculated:', epsilonSH);
...
function calculateepsilonAU(cementType, t, tPrime, t0, T, h, fc, vS, c, wC, aC, specimenShape, aggregateType) {
    ...
    console.log('epsilonAU calculated:', epsilonAU);
...
function handleCalculate() {
    try {
        ...
        // 提取数据并进行计算
        const epsilonSHDataPoints = fileData.slice(1).map((row, index) => {
            if (!Array.isArray(row) || row.length < 13) {
                throw new Error(`第 ${index + 1} 行数据格式无效，需要13列数据`);
            }
            return {
                x: parseFloat(row[1]) - parseFloat(row[2]),
                y: parseFloat(calculateepsilonSH(...row))
            };
        });

        const epsilonAUDataPoints = fileData.slice(1).map((row, index) => {
            return {
                x: parseFloat(row[1]) - parseFloat(row[2]),
        ...
        const JDataPoints = fileData.slice(1).map((row, index) => {
            return {
                x: parseFloat(row[1]) - parseFloat(row[2]),
                ...
                datasets: [
                    {
                        ...
                        backgroundColor: getComputedStyle(document.documentElement).getPropertyValue('--bg-gradient-2').trim(),
                        pointRadius: 4
                    },
                    {
                        label: 'epsilonAU',
                        data: epsilonAUDataPoints,
                        backgroundColor: getComputedStyle(document.documentElement).getPropertyValue('--bg-gradient-1').trim(),
                        pointRadius: 4
                    },
                    {
                        label: 'J',
        ...
        window.currentChart = new Chart(ctx, options);

        showFeedbackMessage('计算完成', 'calculationFeedback');
    } catch (error) {
        console.error('计算过程中出错:', error);
        showFeedbackMessage('计算错误: ' + error.message, 'calculationFeedback');
    }
}
...
function handleExport() {
    ...
    showFeedbackMessage('结果导出完成', 'exportFeedback');
}
